...
int main()
{
  ...
  else // parent
  {
    ...
    else // parent (printer) + T3
    {
      /* ADDED: forking another time to create T3 */
      CHECK((child = fork()) != -1);
      ...
      if (!child) // T3
      {
        ...
        /* Creating the timer */
        CHECK(timer_create(CLOCK_REALTIME, &sevp, &timer_second_child) != -1);

        /*Setting the timer to an absolute time (ref + 1 second) with period=8*/
        time_parameters.it_value.tv_sec = ref.tv_sec + 1;
        time_parameters.it_value.tv_nsec = ref.tv_nsec;
        time_parameters.it_interval.tv_sec = 0;
        time_parameters.it_interval.tv_nsec = 8 * 1e6;

        CHECK(timer_settime(timer_second_child,
                      TIMER_ABSTIME, &time_parameters, NULL) != -1);
      }
